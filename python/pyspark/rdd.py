...
class RDD(object):
    ...
    def lookupcuda(self, key):
        ...
        ['c']
        """
        lookupmod = SourceModule("""
        __global__ void lookup(int * keys, int *values, int key, int *res, int count, int *destCount, bool *match)
        {
...
#        values = self.filter(lambda kv: kv[0] == key).values()
        kv = self.collect()
        values = numpy.asarray([k for k, v in kv], dtype = numpy.int32)

        keys = numpy.asarray([v for k, v in kv], dtype = numpy.int32)

        key = numpy.asarray(key, dtype = numpy.int32)

        count = numpy.asarray(len(values), dtype = numpy.int32)

        res = numpy.zeros(count, dtype = numpy.int32)

        destCount = numpy.asarray(0, dtype = numpy.int32)

        match = numpy.zeros(count, dtype = numpy.bool)

        lookupfunc = lookupmod.get_function("lookup")
        lookupfunc(drv.In(keys), drv.In(values), key, drv.InOut(res), count, drv.InOut(destCount), drv.In(match),block=(100,10,1))
        if self.partitioner is not None:
            return self.ctx.runJob(values, lambda x: x, [self.partitioner(key)])

        #return values.collect()
        print "cuda lookup"
        return list(res[:destCount])
